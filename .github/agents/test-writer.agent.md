...
tools:
  ...
  - Grep
handoffs:
  - label: "Continue: code-reviewer"
    agent: code-reviewer
    prompt: Continue the pipeline. Read harness/queue/agent-queue.json for context and run the code-reviewer stage.
    send: false
---

<!-- GENERATED BY `harness sync` — edit harness/agents/test-writer.md instead. -->
